           ...
           */
           ...
           if($data_tradeno==null||$data_tradeno==""){
               if($_POST['trade_status'] == 'TRADE_FINISHED' || $_POST['trade_status'] == 'TRADE_SUCCESS') {
                   ...
                   $this->assign('message','充值发生了错误:'.$_POST['trade_status']);
                   $this->redirect('Index/index');//跳转到配置项中配置的支付失败页面；
               }
           }
        }else{
            //验证失败
    ...
    */
    function returnurl(){
        ...
        //获取支付宝的通知返回参数，可参考技术文档中页面跳转同步通知参数列表
            ...
            $this->log_result('',$_GET."\n");
            /*
            $isfinish=D('Common/Recordrad')->where(array('trade_no'=>$trade_no));
            if($isfinish!=null){
                //echo "该充值已经生效";
		$this->log_result('',"returnurl重复提交:".$out_trade_no."\n");
                $this->redirect('Index/index',array('message'=>'该订单已经充值成功了,请勿重复提交'));
            }
            */
            $data_tradeno=D('Common/Recordrad')->where(array('out_trade_no' => $out_trade_no))->getField('trade_no');
            if($data_tradeno==null||$data_tradeno==""){
                if($_GET['trade_status'] == 'TRADE_FINISHED' || $_GET['trade_status'] == 'TRADE_SUCCESS') {
                    ...
                    //echo "支付成功!";
		    $this->log_result('',"returnurl支付成功:".$out_trade_no."\n");
                    $this->redirect('Index/index');//跳转到配置项中配置的支付成功页面；
                }else {
                    D('Common/Recordrad')->where(array('out_trade_no'=>$out_trade_no))->save(array('trade_status'=>$trade_status));
                    //echo "trade_status=".$_GET['trade_status'];
		    $this->log_result('',"returnurl发生了充值错误:".$out_trade_no."\n");
                    $this->redirect('Index/index',array('message'=>'充值发生了错误:'.$_POST['trade_status']));//跳转到配置项中配置的支付失败页面；
                }
            }
        }else {
            //验证失败
            ...
            D('Common/Recordrad')->where(array('user_id'=>$user_id))->save(array('trade_status'=>'TRADE_FAIL'));
            //echo "支付失败！";
	    $this->log_result('',"returnurl但是已经被notifyurl处理:".$out_trade_no."----".$user_id."---"."\n");
	    D('Member')->login($user_id, false, 1);
            $this->redirect('Index/index');//跳转到配置项中配置的支付失败页面；
        }
    }
