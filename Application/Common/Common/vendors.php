 ...
 * @param string $mid
 */
function alidayu($mobile, $content, $kind, $bkind){
    if($kind == 'regist'){
        $paramkind = "SMS_66685059";
    } else if($kind == 'change'){
        $paramkind = "SMS_81405022";
    }
    if($bkind == 'iosan'){
        $name = "百达加速器";
    } else if($bkind == 'pc'){
    ...
    $req->setSmsParam("{code:'$content',product:'$name'}");
    $req->setRecNum($mobile);
    $req->setSmsTemplateCode($paramkind);
    try {
        $resp = $c->execute($req);
//        $ret = $resp->result->err_code->toString();
