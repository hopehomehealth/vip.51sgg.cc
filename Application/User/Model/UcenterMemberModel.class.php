...
//    protected $connection = UC_DB_DSN;
    ...
    public function register($username, $nickname, $password, $email, $mobile, $type=1)
    {
        ...
        if ($usercenter_member) {
            ...
            if ($result > 0) {
                ...
                //vpn用户注册
                $radresult=D('Common/Radcheck')->register($data,$uid);
                if($radresult===false){
                    //如果注册失败，则回去Radcheck表删除掉错误的记录
                    D('Common/Radcheck')->where(array('id' => $uid))->delete();
                    //如果注册失败，则回去Memeber表删除掉错误的记录
                    D('Common/Member')->where(array('uid' => $result))->delete();
                }
                action_log('reg','ucenter_member',1,1);
                //记录vpn注册
                action_log('reg','radcheck',1,$radresult);
     ...
     * @auth 陈一枭
     */
    public function changePassword($old_password, $new_password, $dectoken)
    {
        $uid = $dectoken ? $dectoken : get_uid();
        //检查旧密码是否正确
